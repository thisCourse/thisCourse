...
addScores = =>
    analytics.db.collection("posttest").find(type: "proberesponse", email: {$in: students}, timestamp: {$gte: start}).toArray (err, proberesponses) =>
        proberesponses.forEach (response) =>
            ...
            analytics.db.collection("posttest").save(response)
...
analytics.db.collection("posttest").group(
    {email:true}
    ...
    (err, people) =>
        for person in people
            ...
            if person.email in students
                api.db.collection("grade").save points: person.score, email: person.email, title: "Posttest"
            
)

#mongoexport --db analytics --collection midterm -q '{"type":"proberesponse"}' -o midterm.json --jsonArray
#mongoexport --db courses --collection grade -q '{"title":"Posttest"}' -o posttestgrades.json --jsonArray
