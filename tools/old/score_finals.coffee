...
addScores = =>
    analytics.db.collection("final").find(type: "proberesponse").toArray (err, finalresponses) =>
        finalresponses.forEach (response) =>
            ...
            analytics.db.collection("final").save(response)
...
gradeboundaries = midtermgradeboundaries

grades = ['A+','A','A-','B+','B','B-','C+','C','C-','D','F']

analytics.db.collection("final").group(
    {email:true}
    {type:"proberesponse"}
    ...
    (err, people) =>
        for person in people
            ...
            if person.email in students
                ...
                api.db.collection("grade").save points: person.score, grade: grade, email: person.email, title: "Final"
